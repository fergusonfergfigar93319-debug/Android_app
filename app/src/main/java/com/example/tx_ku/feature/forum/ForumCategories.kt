 ...
 * 主题侧重：王者电竞 IP 场景与城市文旅、潮流表达在同一广场里可发现、可发帖。
 */
object ForumCategories {
    ...
    /** 电竞 IP × 城市文旅、潮流打卡、观赛漫游（与首页「文旅」策展同源叙事） */
    ...
    fun categorySearchBlob(categoryId: String): String {
        val extra = when (categoryId) {
            RECRUIT -> "招募 组队 开黑 固玩 找队友 拼车 缺人 招募帖 王者 五排 巅峰 同城 本城 面基 上车"
            GUIDE -> "攻略 教程 心得 教学 技巧 打法 版本 出装 意识"
            SOCIAL -> "闲聊 水贴 交友 扩列 吐槽 树洞 潮流 穿搭 二创 梗图 谷子 周边 同好 扩列"
            EVENT -> "赛事 活动 比赛 联赛 校园 线下 观赛 报名 KPL 主场 城市 文旅 打卡 联动 观赛搭子 粉丝聚会"
            CULTURE -> "电竞文旅 城市漫游 主场 打卡 观赛攻略 潮流 应援 穿搭 二创 文旅联动 同城观赛 粉丝聚会 王者电竞 记录 游记 动线"
            else -> ""
        }
    ...
    fun suggestedTagsForCategory(categoryId: String): List<String> {
        val base = when (categoryId) {
            ...
            SOCIAL -> listOf(
                ...
                "吐槽", "随缘", "夜猫子", "摸鱼",
                "树洞", "固聊", "KPL", "观赛吐槽",
                "潮流穿搭", "梗图二创", "谷子交流", "同好扩列", "王者电竞"
            )
            EVENT -> listOf(
                "KPL", "挑战者杯", "校园", "线下", "观赛", "报名", "友谊赛", "战队",
                "奖励", "校赛", "联赛",
                "城市打卡", "文旅联动", "同城观赛", "主场见", "粉丝聚会", "王者电竞"
            )
            CULTURE -> listOf(
                "城市漫游", "观赛日记", "应援穿搭", "文旅打卡", "主场见",
                "同城观赛", "赛后复盘局", "潮流二创", "王者电竞", "KPL",
                "轻旅行", "观赛礼仪", "粉丝聚会"
            )
            else -> emptyList()
        }
    ...
    fun fullTagPickerAllTags(): List<String> {
        ...
        for (cat in listOf(RECRUIT, GUIDE, SOCIAL, EVENT, CULTURE)) {
